...
class TicTacToe {
  ...
  promptNextMove() {
    if (this.currentPlayer === this.playerTwo) {
      const nextAIMove = this.generateNextAIMove();
      console.log(`\nThe ${this.currentPlayer.name} (${this.currentPlayer.move}) places a move at ${nextAIMove}.`);
      this.makeMove(nextAIMove);
    } else {
      console.log(`\nIt is your (player ${this.currentPlayer.name} (${this.currentPlayer.move})) move. Please enter a number from 1 through 9.\n\n>>`);
    }
  }

  generateNextAIMove() {
    let nextMove = Math.floor(Math.random() * 9);
    while(this.hash[nextMove] !== 0) {
